...
export function MotorEndStateSelect(props: {
  ...
  return (
    <>
      ...
      <select id={motorEndStateId} value={props.endState} onChange={handleChange}>
        {endStateOptions.map((option) => (
          <option key={option.endState} value={option.endState}>
            {option.name}
          </option>
        ))}
      </select>
