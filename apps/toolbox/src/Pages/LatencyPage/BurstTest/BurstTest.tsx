...
import { MotorBufferModeSelect, NoFeedbackToggle, PortIdInput } from '../../../Components';

export function BurstTest(props: {
  hub: IHub | undefined;
  ...
  const execute = useCallback((): void => {
    ...
    const tasks = Array.from({ length: burstLength }, (_, index) => {
      return new Observable((subscriber) => {
        ...
        const sub = props.hub.motors
          .startSpeed(portId, index, { bufferMode, noFeedback })
          .pipe(take(1), takeUntil(disposeRef$.current))
          .subscribe({
    ...
    concat(...tasks, props.hub.motors.startSpeed(portId, 0, { bufferMode, noFeedback }))
      .pipe(takeUntil(disposeRef$.current))
      .subscribe();
  ...
  return (
    <>
      ...
      <div>
        <NoFeedbackToggle noFeedback={noFeedback} onNoFeedbackChange={setNoFeedback} />
      </div>
      <div>
        <button onClick={execute} disabled={portId === undefined || !props.hub}>
          Execute
        </button>
      </div>
    </>
