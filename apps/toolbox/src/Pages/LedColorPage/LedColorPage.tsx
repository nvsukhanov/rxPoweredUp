...
export function LedColorPage(props: { hub?: IHub }): ReactElement {
  ...
  const updateLedColor = (): void => {
    ...
    props.hub.rgbLight.setRgbColor(portId, color, modeId).pipe(takeUntil(dispose$.current)).subscribe();
  };
  ...
  return props?.hub ? (
    <>
      ...
      <button onClick={updateLedColor} disabled={portId === undefined || modeId === undefined || color === undefined}>
        Set color
      </button>
    </>
  ) : (
