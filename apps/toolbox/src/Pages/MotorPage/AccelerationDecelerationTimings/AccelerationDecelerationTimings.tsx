...
export function AccelerationDecelerationTimings(props: { hub: IHub; port: number | undefined }): ReactElement {
  ...
  const handleApply = (): void => {
    if (props.port !== undefined) {
      ...
      props.hub.motors
        .setAccelerationTime(props.port, accelerationTimeMs)
        .pipe(concatWith(props.hub.motors.setDecelerationTime(props.port, decelerationTimeMs)))
        .subscribe({
  ...
  return (
    <>
      ...
      <div>
        <button disabled={!isValid() || isInProgress} onClick={handleApply}>
          Apply
        </button>
      </div>
    </>
