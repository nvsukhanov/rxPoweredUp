...
export function PositionInfo(props: { hub: IHub; portId: number | undefined }): ReactElement {
  ...
  const handlePosRead = (readType: PortModeName.position | PortModeName.absolutePosition): void => {
    ...
    props.hub.ports
      .getPortValue(props.portId, modeId, valueTransformer)
  ...
  const handleSubscribe = (readType: PortModeName.position | PortModeName.absolutePosition): void => {
    ...
    props.hub.ports
      .portValueChanges(props.portId, modeId, 1, valueTransformer)
      .pipe(
        takeUntil(dispose$),
