...
import { SpeedInput, SpeedOptionsForm, SpeedOptionsFormResult } from '../../../Components';

export function SpeedControl(props: { hub: IHub; portId: number | undefined }): ReactElement {
  ...
  return (
    <>
      <div>
        <div>
          <SpeedOptionsForm initialState={speedOptions} onChanges={setSpeedOptions} />
        </div>
        <div>
          <button disabled={!canExecute} onClick={(): void => handleQuickSetSpeed(-100)}>
            -100
          </button>
          <button disabled={!canExecute} onClick={(): void => handleQuickSetSpeed(-50)}>
            -50
          </button>
          <button disabled={!canExecute} onClick={(): void => handleQuickSetSpeed(0, 0)}>
            0 (float)
          </button>
          <button disabled={!canExecute} onClick={(): void => handleQuickSetSpeed(0, 100)}>
            0 (brake)
          </button>
          <button disabled={!canExecute} onClick={(): void => handleQuickSetSpeed(50)}>
            50
      ...
      <div>
        ...
        <button disabled={!canExecute} onClick={handleSetSpeed}>
          Set speed
        </button>
      </div>
    </>
