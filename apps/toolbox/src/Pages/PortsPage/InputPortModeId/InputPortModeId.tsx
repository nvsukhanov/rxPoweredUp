...
import styles from './InputPortModeId.module.scss';
import { PORT_MODE_INFORMATION_TYPES_LIST } from '../port-mode-information-types-list';

export function InputPortModeId(props: {
  hub: IHub;
  portId: number;
  modeId: number;
  ...
  const handleReadPortValueRequest = (): void => {
    ...
    if (transformer) {
      props.hub?.ports
        .getPortValue(props.portId, props.modeId, transformer)
        .subscribe((r) => processPortValue(props.portId, props.modeId, JSON.stringify(r, null, 2)));
    }
  };
    ...
    <section>
      ...
      <dl className={styles['modeValuesContainer']}>
        ...
        <dt className={styles['portValuesContainer']}>
          <div>as</div>
          <div>
            <select value={transformerIdx} onChange={handleTransformersChange}>
          ...
          <div>{portValues?.parsedValue !== undefined ? portValues.parsedValue : '---'}</div>
        </dt>
