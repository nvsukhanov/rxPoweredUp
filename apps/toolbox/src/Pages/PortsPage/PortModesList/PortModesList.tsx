...
import styles from './PortModesList.module.scss';
import { PortModeState, useHubStore } from '../../../Store';
import { InputPortModeId } from '../InputPortModeId';
import { OutputPortModeId } from '../OutputPortModeId';

export function PortModesList(props: { hub: IHub; portId: number; portModeState: PortModeState }): ReactElement {
    ...
    <section>
      ...
      {props.portModeState.inputModes.length > 0 && (
        <section>
          ...
          <section className={styles['modesList']}>
            {props.portModeState.inputModes.map((modeId, idx) => (
              <Fragment key={idx}>
                <InputPortModeId
                  hub={props.hub}
                  portId={props.portId}
      ...
      {props.portModeState.outputModes.length > 0 && (
        <section className={styles['modesSection']}>
          ...
          <section className={styles['modesList']}>
            {props.portModeState.outputModes.map((modeId, idx) => (
              <Fragment key={idx}>
                <OutputPortModeId
                  portId={props.portId}
                  modeId={modeId}
