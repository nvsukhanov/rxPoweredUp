...
export class TaskQueue implements IDisposable {
  ...
  public enqueueTask(task: IQueueTask<unknown>): void {
    if (this.isDisposed) {
  ...
  private executeTask(task: IQueueTask<unknown>): void {
    of(null)
      .pipe(
        switchMap(() => {
          return merge(
  ...
  private removeTaskFromQueue(task: IQueueTask<unknown>): void {
  ...
  private createRetryConfig(task: IQueueTask<unknown>): (error: Error, retryCount: number) => Observable<unknown> {
  ...
  private logTimeoutError(task: IQueueTask<unknown>): void {
    this.logger.warn(`send timeout: ${formatMessageForDump(task.message)}, will retry`);
  }

  private logMaxRetriesReachedError(task: IQueueTask<unknown>): void {
