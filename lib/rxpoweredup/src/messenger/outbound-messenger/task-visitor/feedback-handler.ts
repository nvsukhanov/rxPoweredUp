...
export class FeedbackHandler {
    ...
    public handlePortOutputCommandFeedback(
        ...
        if (feedback.currentCommandDiscarded) {
            const command = this.getFirstPortOutputCommandWithMatchingState(
                commands,
                this.inProgressOrWaitingForResponse
            );
        ...
        if (feedback.busyOrFull) { // weird, somehow that indicates that all remaining tasks are completed
