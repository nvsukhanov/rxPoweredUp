...
export interface IPortsFeature {
    ...
    // OR
    // Seems like it is possible to use well-known modeIds and portModeNames, e.g. 0 and 'absolutePosition' for motors.
    // But this method is brittle and may not work for all devices and be broken in future firmware versions.
    requestPortValue(
        portId: number,
        modeId: number,
